...
export default function Profile({ isSetup }) {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    setProfile({ name: name.trim(), avatar, createdAt: state.profile?.createdAt || Date.now() });
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };

  const handleReset = () => {
    if (confirm('すべてのデータをリセットしますか？ / Bạn có chắc muốn xóa toàn bộ dữ liệu?')) {
      setState(() => ({ profile: null, xp: 0, streak: 0, lastStudyDate: null, progress: { vocab:{seen:[],mastered:[]}, kanji:{seen:[],mastered:[]}, grammar:{seen:[],mastered:[]}, conjugation:{seen:[],mastered:[]}, keigo:{seen:[],mastered:[]}, adverbs:{seen:[],mastered:[]} }, testHistory: [], lastTestResult: null }));
    }
  };
      ...
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        {isSetup && (
          <div className="text-center mb-xl">
            ...
            <p className="text-muted mt-sm">Hãy tạo hồ sơ để bắt đầu hành trình chinh phục JLPT N3!</p>
          </div>
        )}
              ...
              <div className="card card-body mb-lg">
                ...
                {testHistory.map((t, i) => (
                  <div key={i} className="flex justify-between items-center mb-sm" style={{padding:'8px 0',borderBottom:'1px solid var(--pale)'}}>
                    ...
                    <span className="font-bold" style={{color: t.score >= 95 ? 'var(--spring-green)' : 'var(--sakura-700)'}}>
                      {t.score}/120
                    </span>
                    <span className={`badge ${t.score >= 95 ? 'badge-green' : 'badge-error'}`}>
